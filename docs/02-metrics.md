...
- **proxy_for** makes the card a proxy: the id of the real metric's card it stands in for (`15-proxy-ladder.md`). A proxy is listed as a goal; its screen and confirm levels, the exact test, and the ratchet apply to it as to any goal. `sb campaign start` refuses a goal or guardrail whose `proxy_for` is not in the campaign's `audits` list.
- **trust** is engine-owned: `provisional` (set at `campaign start` when absent), `validated`, `suspect`, `demoted`, moved only by audits (`13-statistical-guarantees.md` §13.9 for what the labels mean). It is excluded from the card fingerprint, so the engine's own change to it does not halt the campaign as tampering. A demoted proxy no longer covers any target at pre-registration; it is still measured.
- **covers** is the proxy's scope: repo-relative path patterns (`dir/` prefix, glob, exact path) it is valid for. In a campaign with `audits`, `sb prereg` refuses a hypothesis whose `target` (before the first `:`) matches no non-demoted proxy's `covers`, unless some non-demoted proxy has no `covers` at all. Distinct from `targets`, which stays the code whose change moves the metric.
- **audit** makes the card the real instrument of a proxy ladder, listed under the campaign's `audits`. `{"every_accepts": 3, "discard_sample_rate": 0.1, "pairs": 3, "alpha": 0.05}`; every key defaults to the constant shown. The card keeps `kind: goal` and its ordinary `measure`; an audit measures it at its `confirm` fidelity (command, timeout, holdout) with `pairs` interleaved pairs. It may not also be a goal or guardrail of the campaign.
...
- **audits** lists the real instruments of a proxy ladder (`15-proxy-ladder.md`); empty by default. Wiring rules at start: every card in `audits` must carry an `audit` object and may not also be a goal or guardrail; every goal or guardrail with `proxy_for` must name a card in `audits`; a proxy without `trust` is set to `provisional`. Audit cards are part of the frozen set (fingerprinted, their `frozen_paths` hashed) and are baselined at start like the other cards, which runs the real instrument `k` times per level the card measures (`screen` and `confirm`, plus `full` when defined; `fidelity.screen.skip: true` limits it to `confirm`) unless `--no-baseline`. They are not checked for a valid baseline or a measured sigma, and `start_values` does not record them. Their ratchet moves only at audits (§2.5). A hypothesis may not `predict` an audit card.
...
- **composition: frontier** is for two or more goals that trade off (speed against cost, latency against accuracy); `sb campaign start` refuses it with one goal. The campaign keeps an archive of non-dominated commits, the **members**. `f0` is the base commit; every later member `f<k>` sits on its own branch `sb/<campaign>-f<k>`, and `sb/<campaign>` points at the **preferred member**. With `preference: {"weights": {goal: w}}` that is the member with the largest weighted sum of normalized gains over the base, `s_g · (m_g(member) − m_g(f0)) / |m_g(f0)|` per goal; without weights it is the knee, the member whose smallest normalized gain over the base is largest. The base scores zero on every goal, so without weights a member that trades one goal away is never preferred over `f0`; declare weights when a trade should win. Each experiment branches from a member the engine picks: fewest attempts, ties to the largest NSGA-II crowding distance, so the extremes are explored first. `sb prereg --parent f2` overrides. At screen a guardrail regression discards and a goal regression does not (it may be a trade); the screen still compares against the stored baseline of the base commit, because a frontier campaign never rewrites `baseline.json`. At confirm the exact test of §2.4 runs paired against the parent member, measured fresh: the candidate must improve at least one goal (`p ≤ α_look`, practical floor held) and break no guardrail, and it may regress other goals. It is then compared with every active member on stored confirm medians. If some member is not worse than it on any goal beyond `τσ` and better on at least one beyond `κσ` (σ the stored confirm-level sigma), the candidate is dominated and the verdict is `discard dominated:<member>`. Otherwise it joins as `f<k>`, every member it dominates retires (`retired_by: f<k>`), and the archive is pruned to `frontier_max` (default 8) by crowding distance (`retired_by: crowding`; a member that is the extreme on some goal is pruned only when nothing else is left). A frontier campaign writes neither `baseline.json` nor `ratchet.json` (§2.5): the frontier is the deliverable, and a human picks a point at gate 2 unless `preference` already did. `sb frontier [--json]` lists the members and the report has a Frontier table. `06-search.md` §6.9 says when to choose this over a tolerance or an OEC; `13-statistical-guarantees.md` §13.7 says which parts of the rule carry an error rate.
...
The **global ratchet** lives in `ratchet.json`: one entry per metric that has ever been a goal in any campaign on this project, holding `best`, `sigma`, `commit`, `campaign`, and `direction`, written by `sb accept`. `sb campaign start` adds every ratcheted metric that has a card as a guardrail (at its best confirmed value, with its tolerance) unless its entry carries `"demoted": true`. A frontier campaign (§2.3) writes no ratchet entry: its members are a mapped trade-off, not floors. An audit card of a proxy ladder (§2.3, `15-proxy-ladder.md`) is ratcheted only by an accept or end audit whose verdict is `confirmed` or `direction`; its `baseline.json` and `ratchet.json` entries then carry `audited: true` and the ratchet entry `audit_verdict`. `sb accept` itself never moves an audit card. Consequences:
