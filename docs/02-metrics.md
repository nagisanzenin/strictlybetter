...
Every field except `goals` has a default: `budget` is `{"experiments": 40}`, `branch` is `sb/<id>`, `plateau_patience` is 8, `max_parallel` is 2, `distill_every` is 8, `iteration_cap` is 200, every wall is true, `external_instruments` and `scope_paths` are empty (no instrument outside the repo; the whole repo in scope), and `services` is `null`. A `services` object has `setup`, `ready`, `teardown`, `cwd` (default: the checkout), `ready_timeout_s` (120), `ready_interval_s` (2), `setup_timeout_s` (600), and `teardown_timeout_s` (300); `07-universality.md` §7.7 has the semantics. The engine writes the result to `.strictlybetter/campaign.json` together with the fields it owns: `status` (`running` | `halted` | `ended`), `halt_reason`, `base_commit`, `head_commit`, `eval_hash`, `frozen_paths_effective`, `external_instruments_effective`, `external_hashes`, `spent`, `mde` (each goal's minimum detectable effect), `exploration_level`, `since_last_accept`, `accepted_ids`, `consecutive_integrity`, `consecutive_gamed`, `screen_repeats_multiplier`, `screen_untrusted`, `holdout_override`, `next_id`. Only the harness writes this file.
...
Defaults: `κ = 2.5`, `τ = 1.0`, overridable per card in `acceptance`. In the engine: a goal *improves* when `Δ > κ_eff · σ · f`, *regresses* when `Δ < −τ · σ · f`, and is *inconclusive* when `0 < Δ ≤ κ_eff · σ · f`, where `f = √(1/r + 1/k)` scales sigma to a comparison of a candidate median of `r` repeats against a baseline median of `k` repeats: at screen (`r = 1`, `k = 5`) the factor is 1.10, at confirm (`r = 3`, `k = 5`) it is 0.73, so confirmation is the more sensitive comparison. With the `paired` wall on (the default), the `m_x(A)` of (1) and (2) at confirmation is not the stored baseline but a fresh measurement of the campaign head taken in the same `sb confirm` run, interleaved with the candidate's repeats; `σ_x` stays the baseline's and `k` becomes the fresh head run's valid repeat count, so with `r = 3` on both sides the factor is 0.82 (`04-anti-overfitting.md` §4.2). The `σ` here is the robust estimate from `sb baseline`: 1.4826 × the median absolute deviation when the metric has at least four repeats, the sample standard deviation for two or three (`04-anti-overfitting.md` §4.2). A sigma of zero makes any move decisive. Larger diffs raise κ: `κ_eff = κ · (1 + 0.3 · ln(1 + diff_lines / 50)) + 1.0 · σ` per touched dependency manifest (the complexity regularizer in `04-anti-overfitting.md`).
