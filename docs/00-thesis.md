...
**C3. Noise is the main enemy of an unattended loop.** Every metric has an empirically measured noise floor from repeated baseline runs. An improvement smaller than the noise floor is not an improvement. This single rule removes most false positives that an autoresearch-style loop would otherwise merge. See `04-anti-overfitting.md`.

**C4. Overfitting in a code loop means optimizing the instrument instead of the property.** The defenses are layered because each catches a different class: noise floor plus confirmation, a frozen instrument the experimenter cannot edit, a holdout the experimenter never sees, pre-registration of the hypothesis before the run, and a blind judge that reads the diff for gaming patterns. See `04-anti-overfitting.md`.
