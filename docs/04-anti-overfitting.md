...
| 1 paired confirmation | `paired` | `sb confirm` compares the candidate against the stored confirm-level baseline instead of a fresh, interleaved measurement of the campaign head; the head worktree is not created and confirmation costs half |
...
**Threshold.** A goal must move by more than `κσ` (κ = 2.5 default). This is deliberately not a p-value from a two-sample test; with 3 to 5 repeats per side those tests are unreliable, and an effect-size threshold in sigma units is what practitioners actually use in performance CI (the Rust compiler's perf triage uses a significance threshold in the same spirit). In the engine the threshold is `κ_eff · σ · √(1/r + 1/k)`, because the comparison is a candidate median of `r` repeats against a baseline median of `k`; at screen (`r = 1`, `k = 5`) the factor is 1.10, at confirm (`r = 3`, `k = 5`) it is 0.73. The guardrail tolerance `τσ` is scaled the same way. At campaign start the engine also computes each goal's **minimum detectable effect**, `κ · σ · √(1/r + 1/k) / |median|` with `r` the confirm repeats and `k` the baseline repeats, prints it, stores it in `campaign.json` under `mde`, and halts with `instrument-unusable:<metric>:mde=…` when it exceeds 50%; the message tells the user to raise repeats, reduce machine load, or pass `--allow-unusable`. This is the "the instrument is the problem" rule of `06-search.md` §6.8 made concrete before any experiment is spent: a goal the instrument cannot see moving is not a goal on this host.

**Confirmation.** The screening number that promoted a candidate is discarded for the purpose of acceptance. `sb confirm` re-measures from a clean checkout of the experiment commit: first at `full` fidelity for every card that defines it (a discard there ends the candidate), then at `confirm` fidelity with holdout inputs and `r` repeats (default 3; the card's `max_repeats` when the anomaly flag is set or the judge said `suspicious`). Acceptance uses the confirmation median. This is the winner's-curse correction: the number that selected the winner is biased upward by selection, the fresh number is not.

**Paired against a fresh head.** What the confirmation median is compared against matters as much as the median itself. The stored baseline was measured when the campaign started or last ratcheted; by the time a candidate is confirmed the machine may be under a different load, thermally throttled, or warmed differently. The bench showed exactly that drift producing a false accept and a no-op accept on a loaded machine. With the `paired` wall on (the default), `sb confirm` checks the campaign head out into a second worktree and, at `full` and `confirm` fidelity, measures head and candidate interleaved: repeat by repeat, in ABBA order (head then candidate, then candidate then head), with the same holdout value on both sides, so a load burst or a clock change hits both alike. The candidate's median is then compared against the head's fresh median. `σ` still comes from the `k`-repeat baseline, and the `k` in the threshold's `√(1/r + 1/k)` is the fresh head run's valid repeat count. An inconclusive round adds repeats to both sides. If the head's fresh measurement is itself invalid, the comparison falls back to the stored baseline. This is standard performance-CI practice: rustc-perf measures the baseline in the same job as the candidate, and Chromium's Pinpoint runs paired A/B jobs. The confirm ledger event records `paired: true` and the head's summaries as `head_results`; the provenance block prints `paired=yes`. The price is that confirmation takes about twice as long (`05-cost-and-speed.md` §5.2).
