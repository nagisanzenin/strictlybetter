...
2. **A demoted proxy is still measured but never decides: `confirm_decision` skips it and `decide` ignores its screen improvement; its `covers` no longer admit targets; the campaign halts only when no confirming proxy is left.** As built, "stops confirming" is enforced at pre-registration: a demoted proxy no longer covers any target, so no new experiment can rest on it. It is still measured at confirm for experiments already open. The halt fires when every goal is a demoted proxy.
