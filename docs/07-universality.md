...
2. **Holdout is a slice of the problem space.** Confirmation runs use parameter settings, seeds, or datasets the experimenter never saw. The same exact paired test applies, with one holdout parameter setting per pair, run on candidate and head alike (`13-statistical-guarantees.md`). A method that only works on the development case is discarded.
