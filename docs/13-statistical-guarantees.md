...
Since 1.2.1 the per-confirmation α is split Bonferroni across the G goals tested in that confirmation (`α_goal = α_look / G`; OEC tests one composite at `α_look`). The bound below therefore holds per confirmation regardless of the number of goals, and the family-wise bound over the campaign follows from the split over the pre-registered K. Formulas: `docs/14-mathematics.md` §14.4.
...
## 13.7 What the external-adversary gate showed (2026-09-03)
...
## 13.8 Frontier campaigns: which parts of the rule carry an error rate
...
The preferred point without weights is the knee over the frontier's own extremes (`docs/14` §14.7): the most balanced active member, so a pure trade can be preferred. With weights it is the weighted sum of normalized positions. Either is a decision aid, not a claim.
