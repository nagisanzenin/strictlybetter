...
MUTATIONS = [
    ...
    {"name": "holdout: use_holdout=False in cmd_confirm", "cls": "5 holdout",
     "old": 'use_holdout=walls.get("holdout", True)', "new": "use_holdout=False", "count": 2,
     "expect": []},
    ...
    {"name": "validity: timer-divergence check disabled in comparisons_for", "cls": "validity",
     "old": "instr_ratio < WALL_DIVERGENCE_INSTR", "new": "False", "count": 1,
     "expect": []},
    ...
    {"name": "budget: experiments charged after worktree_new in cmd_prereg", "cls": "8 budget",
     ...
     "expect": []},
    {"name": "label: dollars_est -> dollars in stats (table sed)", "cls": "9 label",
     "old": '"dollars_est": round(dollars, 4)', "new": '"dollars": round(dollars, 4)', "count": 1,
     "expect": []},
    {"name": "label: report drops the word `estimated`", "cls": "9 label",
     "old": 'f"- estimated dollars (from reported tokens): {s[\'dollars_est\']}"', "new": 'f"- dollars: {s[\'dollars_est\']}"', "count": 1,
     "expect": []},
