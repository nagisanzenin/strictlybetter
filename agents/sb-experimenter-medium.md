---
name: sb-experimenter-medium
description: "strictlybetter experimenter pinned to MEDIUM reasoning effort. MUST BE USED by /strictlybetter:run for the algorithmic, allocation, caching, bugfix, numerics, and data operators: replace an algorithm or data structure in one function, remove copies, memoize, fix a root cause. Fresh-context per experiment: implements exactly one pre-registered hypothesis inside one worktree, sees no other experiment, no holdout, no confirm numbers; returns DONE <id> or BLOCKED: <reason>. Byte-identical to the other sb-experimenter agents except this frontmatter."
